<div class="container">
    <div class="pt-20">
        <div class="flex -mx-4">
            <div class="flex-1 px-4">
                <div class="bg-white rounded-xl flex">
                    <div style="min-width: 130px" class="bg-gray-500"></div>
                        <div class="px-4 py-4">
                            <h3 class="mb-4 text-xl">Lorem ipsum dolor sit amet Lorem ipsum..</h3>
                            <a href="" class="fullwidth-btn"></a>
                        </div>
                </div>
            </div>
            <!-- End of box-->
            <div class="flex-1 px-4">
                <div class="bg-white rounded-xl flex">
                    <div style="min-width: 130px" class="bg-gray-500"></div>
                        <div class="px-4 py-4">
                            <h3 class="mb-4 text-xl">Lorem ipsum dolor sit amet Lorem ipsum..</h3>
                            <a href="" class="fullwidth-btn"></a>
                    </div>
                </div>
            </div>
            <!-- End of box-->
            <div class="flex-1 px-4">
                <div class="bg-white rounded-xl flex">
                    <div style="min-width: 130px" class="bg-gray-500"></div>
                        <div class="px-4 py-4">
                            <h3 class="mb-4 text-xl">Lorem ipsum dolor sit amet Lorem ipsum..</h3>
                            <a href="" class="fullwidth-btn"></a>
                    </div>
                </div>
            </div>
            <!-- End of box-->

        </div>
    </div>
    <!-- End of Article and Advice Start of Advertisement-->
    <div class="pt-20 text-lg border-b-2 border-gray-300 pb-16 mb-20">
        <h2 class="section-heading">Additional Services</h2>
        <div class="flex -mx-4">
            <div class="flex-1 px-4">
                <a href="" class="underline">Lorem ipsum dolor.</a>
            </div>
            <div class="flex-1 px-4">
                <a href="" class="underline">Lorem ipsum dolor.</a>
            </div>
            <div class="flex-1 px-4">
                <a href="" class="underline">Lorem ipsum dolor.</a>
            </div>
            <div class="flex-1 px-4">
                <a href="" class="underline">Lorem ipsum dolor.</a>
            </div>
        </div>
    </div>

    <div class="flex pb-12">
        <div class="pr-6 w-1/4">
            <img style="width: 100px" src="/img/logo.jpeg" alt="">
            <p class="mt-12">Lorem ipsum dolor sit amet.</p>
        </div>

        <div class="w-3/4 font-bold leading-8">
            <div class="flex -mx-6">
                <div class="flex-1 px-6">
                    <ul>
                        <li><a href="">Lorem ipsum.</a></li>
                        <li><a href="">Lorem ipsum.</a></li>
                        <li><a href="">Lorem ipsum.</a></li>
                        <li><a href="">Lorem ipsum.</a></li>
                        <li><a href="">Lorem ipsum.</a></li>
                    </ul>
                </div>
                <div class="flex-1 px-6">
                    <ul>
                        <li><a href="">Lorem ipsum.</a></li>
                        <li><a href="">Lorem ipsum.</a></li>
                        <li><a href="">Lorem ipsum.</a></li>
                        <li><a href="">Lorem ipsum.</a></li>
                        <li><a href="">Lorem ipsum.</a></li>
                    </ul>
                </div>
                <div class="flex-1 px-6">
                    <ul>
                        <li><a href="">Lorem ipsum.</a></li>
                        <li><a href="">Lorem ipsum.</a></li>
                        <li><a href="">Lorem ipsum.</a></li>
                        <li><a href="">Lorem ipsum.</a></li>
                        <li><a href="">Lorem ipsum.</a></li>
                    </ul>
                </div>
            </div>
        </div>

    </div>

</div>
<img class="w-full" src="/img/footer.jpg" alt="">
